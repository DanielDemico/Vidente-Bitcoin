...
def gerar_previsao():
    ...
    # print(f'Mean Squared Error: {mse:.2f}' ) DEIXAR PARA VERIFICAR CASO ALGO OCORRA ERRADO
    # print(f'R² Score: {r2:.2f}') DEIXAR PARA VERIFICAR CASO ALGO OCORRA ERRADO

    preco_atual = pd.DataFrame([[btc_data.iloc[-1]['Close']]], columns=["Close"])  
    previsao_futura = model.predict(preco_atual)

    # print(f'Preço previsto do Bitcoin para o próximo dia: {previsao_futura[0]:.2f}$') DEIXAR PARA VERIFICAR CASO ALGO OCORRA ERRADO
